...
class ScoreCalc(object):
    ...
    def score_print(self, data_num, train=True):
        if train:
            print("train mean loss={}, accuracy={}".format(
                self.losses[0].get_score() * self.batch_size / data_num,
                float(self.corrects[0].get_score() / data_num)))
        else:
    ...
    def score_append(self, data_num, train=True):
        if train:
            for l in range(self.len_l):
                self.train_losses[l].append(self.losses[l].get_score() *
                                            self.batch_size / data_num)
            for c in range(self.len_c):
                self.train_corrects[c].append(
                    float(self.corrects[c].get_score() / data_num))
        else:
            for l in range(self.len_l):
                self.test_losses[l].append(self.losses[l].get_score() *
                                           self.batch_size / data_num)
            for c in range(self.len_c):
                self.test_corrects[c].append(
                    float(self.corrects[c].get_score() / data_num))
