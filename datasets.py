...
class TineyImagenet(torch.utils.data.Dataset):
    def __init__(self,
        ...
        if train:
            ...
            self.label = np.load(root + '/train/label_train.npy').astype(
                np.int64)
            self.label = torch.from_numpy(self.label)
            self.num = len(self.data)
        else:
            self.data = np.load(root + '/test/img_test.npy')
            self.label = np.load(root + '/test/label_test.npy').astype(
                np.int64)
            self.label = torch.from_numpy(self.label)
            self.num = len(self.data)
    ...
    def create(self):
        ...
        if self.dataset_name == "MNIST":
            ...
            trainset = torchvision.datasets.FashionMNIST(
                root='~/work/FashionMNISTDataset/data',
                train=True,
                download=True,
                transform=self.transform)
            ...
            trainset = torchvision.datasets.CIFAR100(
                root='~/work/CIFAR100Dataset/data',
                train=True,
                download=True,
                transform=self.transform)
            ...
            trainset = TineyImagenet(root='~/work/TinyImagenet',
                                     train=True,
                                     transform=self.transform)
            testset = TineyImagenet(root='~/work/TinyImagenet',
                                    train=False,
                                    transform=self.transform)
            ...
            trainset = OlivettiFaces(root='~/work/Olivettifaces/data',
                                     transform=self.transform)
            testset = None
            ...
            trainset = torchvision.datasets.ImageFolder(
                root='~/work/COIL-20/data', transform=self.transform)
            testset = None
        ...
        return [
            trainloader, testloader, classes, base_labels, trainset, testset
        ]
